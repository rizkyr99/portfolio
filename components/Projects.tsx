  ...
  return (
    <section id="projects" className="relative py-24 px-6 bg-cream dark:bg-[#140A04]">
      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.map((p) => (
              <article
                key={p.title}
                className="group relative rounded-2xl bg-white/70 dark:bg-cream/5 border border-rosewood/10 dark:border-cream/10 p-6 shadow-sm hover:shadow-chrome transition-all duration-300"
              >
                <div className="flex items-start justify-between gap-3">
                  <h3 className="font-display text-2xl text-rosewood dark:text-cream leading-tight">
                    {p.title}
                  </h3>
                  <span className="font-mono text-[10px] text-rosewood/50 dark:text-cream/50 uppercase tracking-widest mt-2">
                    {p.year}
                  </span>
                </div>

                <p className="mt-3 text-rosewood/75 dark:text-cream/75 text-[15px] leading-relaxed">
                  {p.blurb}
                </p>
                ...
                <div className="mt-5 flex items-center gap-4 text-rosewood/70 dark:text-cream/70">
                  <Link
        ...
        {filtered.length === 0 && (
          <p className="text-center text-rosewood/50 dark:text-cream/50 font-mono text-sm py-16">
